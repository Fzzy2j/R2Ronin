...
/// from Ronin to plugin unless Ronin can own that memory.
/// This means that plugins should manage their own memory and can only receive data from ronin using one of the functions below.
...
// Ronin -> Plugin
typedef void (*PLUGIN_INIT_TYPE)(PluginInitFuncs* funcs, PluginRoninData* data);
