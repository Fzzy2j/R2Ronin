...
void MasterServerManager::AuthenticateOriginWithMasterServer(const char* uid, const char* originToken)
{
	...
	std::thread requestThread(
		...
		{
			spdlog::info("Trying to authenticate with ronin masterserver for user {}", uidStr);
			...
			if (result == CURLcode::CURLE_OK)
			{
				...
				if (originAuthInfo["success"].IsTrue() && originAuthInfo.HasMember("token") && originAuthInfo["token"].IsString())
				{
					...
					spdlog::info("Ronin origin authentication completed successfully!");
				}
				else
					spdlog::error("Ronin origin authentication failed");
			}
			else
			{
				spdlog::error("Failed performing ronin origin auth: error {}", curl_easy_strerror(result));
