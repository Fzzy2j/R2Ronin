...
void ModManager::LoadMods()
{
	...
	std::ifstream enabledModsStream(GetRoninPrefix() + "/enabledmods.json");
...
void ModManager::UnloadMods()
{
	...
	std::ofstream writeStream(GetRoninPrefix() + "/enabledmods.json");
...
{
	return fs::path(GetRoninPrefix() + MOD_FOLDER_SUFFIX);
}
fs::path GetRemoteModFolderPath()
{
	return fs::path(GetRoninPrefix() + REMOTE_MOD_FOLDER_SUFFIX);
}
fs::path GetCompiledAssetsPath()
{
	return fs::path(GetRoninPrefix() + COMPILED_ASSETS_SUFFIX);
}
