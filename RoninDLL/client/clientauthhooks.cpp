...
// clang-format on
{
	...
	// so, we need to manually invalidate our key until we're authed with ronin, then we'll allow game to auth with stryder
...
ON_DLL_LOAD_CLIENT_RELIESON("engine.dll", ClientAuthHooks, ConVar, (CModule module))
{
	...
	Cvar_ns_has_agreed_to_send_token = new ConVar(
		...
		"whether the user has agreed to send their origin token to the ronin masterserver");
}
