...
{
	std::ifstream banlistStream(GetRoninPrefix() + "/banlist.txt");
	...
	// m_sBanlistStream.open(GetRoninPrefix() + "/banlist.txt", std::ofstream::out | std::ofstream::binary | std::ofstream::app);
}

void ServerBanSystem::ReloadBanlist()
{
	std::ifstream fsBanlist(GetRoninPrefix() + "/banlist.txt");
...
void ServerBanSystem::ClearBanlist()
{
	...
	m_sBanlistStream.open(GetRoninPrefix() + "/banlist.txt", std::ofstream::out | std::ofstream::binary);
	m_sBanlistStream.close();
}
...
void ServerBanSystem::UnbanUID(uint64_t uid)
{
	...
	std::ifstream fs_readBanlist(GetRoninPrefix() + "/banlist.txt");
	...
	if (!fs_readBanlist.fail())
	{
		...
		{
			// support for comments and newlines added in https://github.com/R2Ronin/RoninLauncher/pull/227
