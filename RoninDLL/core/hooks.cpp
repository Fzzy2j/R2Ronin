	...
	if (cmdlineOrg == nullptr || cmdlineModified == nullptr)
	{
		...
		if (!ignoreStartupArgs)
		{
			...
			if (cmdlineArgFile)
			{
				...
				// if some other command line option includes "-ronin" in the future then you have to refactor this check to check with
...
// clang-format on
{
	...
	if (!strncmp(lpLibFileName, "XInput1_3.dll", 14))
	{
		...
		if (!moduleAddress)
		{
			MessageBoxA(0, "Could not find XInput9_1_0.dll", "Ronin", MB_ICONERROR);
			exit(EXIT_FAILURE);
