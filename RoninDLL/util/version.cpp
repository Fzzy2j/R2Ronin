...
{
	constexpr int ronin_version[4] {RONIN_VERSION};
	int ua_len = 0;
	...
	if (ronin_version[3] == 1)
	{
		sprintf(version, "%d.%d.%d.%d+dev", ronin_version[0], ronin_version[1], ronin_version[2], ronin_version[3]);
		ua_len += snprintf(
			NSUserAgent + ua_len,
	...
	{
		sprintf(version, "%d.%d.%d.%d", ronin_version[0], ronin_version[1], ronin_version[2], ronin_version[3]);
		ua_len += snprintf(
			NSUserAgent + ua_len,
			sizeof(NSUserAgent) - ua_len,
			"R2Ronin/%d.%d.%d",
